...
int WINAPI V_Thread_Play(LPVOID lpPara)
{
...
int __stdcall Video_Play(LONG nPort, HWND hWnd, BOOL AudioFlag)
{
    ...
    thread_pause[nPort] = 0;
    thread_exit[nPort] = 0;
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)V_Thread_Play, NULL, 0, NULL); //开启线程
    return 0;
}
...
int WINAPI V_Thread_Record(LPVOID lpPara)
{
...
int __stdcall Video_StartRecord(LONG nPort, const char *sUrl, const char *path, const char *TrainNum, const char *IPCName, int CH)
{
    ...
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)V_Thread_Record, (void *)path, 0, NULL); //开启线程
    return 0;
}
