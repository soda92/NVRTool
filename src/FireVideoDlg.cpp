...
CFireVideoDlg::CFireVideoDlg(CWnd *pParent /*=NULL*/)
    : CDialogEx(CFireVideoDlg::IDD, pParent)
{
...
void CFireVideoDlg::DoDataExchange(CDataExchange *pDX)
{
...
BEGIN_MESSAGE_MAP(CFireVideoDlg, CDialogEx)
ON_WM_PAINT()
ON_WM_QUERYDRAGICON()
ON_WM_DEVICECHANGE()
ON_NOTIFY(TCN_SELCHANGE, IDC_TAB1, &CFireVideoDlg::OnTcnSelchangeTab1)
ON_WM_TIMER()
ON_BN_CLICKED(IDC_BUTTON_STOPWARN, &CFireVideoDlg::OnBnClickedStopWarn)
ON_WM_CTLCOLOR()
ON_WM_ERASEBKGND()
END_MESSAGE_MAP()
...
{
    CFireVideoDlg *dlg = (CFireVideoDlg *)theApp.pMainDlg;
...
BOOL CFireVideoDlg::OnInitDialog()
{
    ...
    {
        std::thread(Thread_TaxData, this).detach();
        SetTimer(3, 20 * 1000, NULL);
    }
    std::thread(Thread_SetIpcTime, this).detach(); //校时 10min
    ...
    {
        std::thread(Thread_PweData, this).detach();
    }
    ...
    std::thread(Thread_Play, this).detach(); //开启视频线程

    //////////////////////////////////////////////////////////////////////////
    BROADCASTInit();
    if (FireUdpInit() != -1)
    {
        std::thread(Thread_UDPBroadcastRecv, this).detach();
    }

    std::thread(Thread_Voice, this).detach();
    std::thread(Thread_Index, this).detach(); // 开启公共信息记录线程
...
//显示。
HCURSOR CFireVideoDlg::OnQueryDragIcon()
{
    return static_cast<HCURSOR>(m_hIcon);
}

void CFireVideoDlg::OnTcnSelchangeTab1(NMHDR *pNMHDR, LRESULT *pResult)
{
    // TODO: 在此添加控件通知处理程序代码
...
int CFireVideoDlg::VideoPlay(char *ip, long &user_id, long &handle, HWND hWnd)
{
...
void CFireVideoDlg::OnTimer(UINT_PTR nIDEvent)
{
...
int CFireVideoDlg::BROADCASTInit()
{
    theApp.BSoc = -1;
...
BOOL CFireVideoDlg::OnDeviceChange(UINT nEventType, DWORD_PTR dwData)
{
    ...
    this->usb_flag = true;
    std::thread(Thread_UDisk_Process, this).detach(); //开启下载线程
    return TRUE;
}

HBRUSH CFireVideoDlg::OnCtlColor(CDC *pDC, CWnd *pWnd, UINT nCtlColor)
{
...
BOOL CFireVideoDlg::OnEraseBkgnd(CDC *pDC)
{
    // TODO: 在此添加消息处理程序代码和/或调用默认值
...
void CFireVideoDlg::OnBnClickedStopWarn()
{
    // TODO: 在此添加控件通知处理程序代码
